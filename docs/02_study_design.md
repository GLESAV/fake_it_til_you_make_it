...
| 2026-08-20 | **Primary experiment re-scoped to the Gemini coverage arm; see `docs/07_coverage_arm.md`.** Pool balanced across severity and Fitzpatrick I-VI rather than matched to ACNE04's class prior. Closed-set SD arm demoted to the baseline the coverage arm must beat. | The original brief was a deliberately wider synthetic set validated against real data, not a matched-budget substitution study. A generator fine-tuned on ACNE04 cannot be more complete than ACNE04, and demonstrably was not: its pool inherited a 267-person, single-cohort distribution. Gemini produces a 4-severity x 6-Fitzpatrick grid from prompts alone at 3.9 cents per image. |
