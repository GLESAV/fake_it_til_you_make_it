...
\title{Most of the test set is already in the training set:\\
subject leakage in dermatology image benchmarks}
\author{}
\date{}
...
\begin{abstract}
Dermatology image datasets routinely contain several photographs of the same patient or
the same lesion. Splitting them by image --- the default everywhere --- therefore puts the
same subject on both sides of the boundary. We measure how often, in three datasets from
three institutions with three different collection protocols, and what it is worth.
\textbf{ACNE04} \citep{wu2019}, whose 1{,}457 photographs turn out to be roughly 550--750
distinct individuals, leaks a person from training into \textbf{15.9\%} of every published
test fold at an identity threshold where 0.019\% of image pairs qualify by chance, and
\textbf{77.5\%} at the ordinary operating point. \textbf{SCIN}, whose published schema makes
subject identity ground truth so that no model is involved at all, leaks \textbf{75.6\%}.
\textbf{HAM10000} leaks \textbf{38.5\%} overall --- but \textbf{68.3\% on melanoma}, the
class its headline metric is reported for, because melanomas were photographed more often
per lesion. Duplication is almost never independent of class, so a single leakage rate
understates the effect where it matters. We quantify the cost on ACNE04: an identical
classifier trained on image-level rather than subject-disjoint splits gains \textbf{4.5
points of balanced accuracy} (95\% CI 2.3--7.0), concentrated almost entirely in the two
rare, clinically important grades ($+9.9$, $+9.0$) and absent from the two majority grades.
Along the way we report three further defects in ACNE04 --- 38 byte-identical duplicate
pairs, eight of them label-conflicting; 4.25\% duplicate leakage in every published fold;
and a severity label that a second expert team's re-annotation of the same photographs
reproduces on only 30.1\% of images (58.2\% after any monotone recalibration) --- and we
show that the standard instruments for detecting all of this are themselves miscalibrated
on homogeneous corpora: the threshold that establishes the field's reference memorisation
rate flags 82.6\% of real ACNE04 images as copies of one another. Every number reproduces
from public data; two of the three leakage measurements need no images and no model.
\end{abstract}

\section{Introduction}

A test set exists to estimate performance on data the model has not seen. In dermatology
that intention is routinely defeated by a property of the data nobody hides: patients and
lesions are photographed more than once. SCIN says so in its schema, HAM10000 ships a
\texttt{lesion\_id} column, and ACNE04's repetition is discoverable in twenty minutes with
a face-recognition model. Splitting by image rather than by subject then puts the same
person, or the same lesion, on both sides of the boundary --- and what a model learns about
a specific individual transfers perfectly to another photograph of them and not at all to a
new patient.

We measure this in three datasets and find between a third and three quarters of the test
set affected, with the leak concentrating on whichever class happened to be photographed
most. On ACNE04 we also measure what it buys: 4.5 points of balanced accuracy, more than
the entire spread between the published methods competing on that benchmark.

Comparable audits exist and we build on them rather than around them.
\citet{fitzpatrick_audit2024} document duplicates, train/test leakage and label errors in
HAM10000, DermaMNIST and Fitzpatrick17k, and release corrected partitions;
\citet{cleanpatrick2025} build a cleaning benchmark on Fitzpatrick17k and report 21\%
near-duplicates and 32\% label errors. What we add is the subject-level framing, the
cross-dataset comparison, the measured cost in accuracy, and the observation that the
instruments used for these audits need calibrating against the corpus they are pointed at.

ACNE04 is our deep case: it has never been audited, it is small enough to examine
exhaustively, and it is the only one of the three with a second independent expert
annotation to compare against. Three of the defects we report there require nothing but
the archive and a hash function.
...
HAM10000 publishes \texttt{lesion\_id}, giving a third metadata-only check. Its duplication
is already documented --- \citet{fitzpatrick_audit2024} found near-duplicate lesions
spanning the train/validation boundary and released corrected DermaMNIST partitions --- and
we do not claim to have found it. What we measure is the size and the \emph{shape} of the
consequence. It leaks 38.5\% overall, about half of SCIN and ACNE04, which follows directly
...
Three datasets are three datasets and we do not claim universality. But they were not
selected for agreement --- SCIN and HAM10000 were chosen precisely because their ground
truth is independent of our instrument --- and the burden now sits with anyone assuming
their own dermatology benchmark is exempt. The check costs nothing: a
