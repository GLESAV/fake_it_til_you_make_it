...
definitional difference. We quantify the cost of (iii): an identical classifier
trained on image-level rather than subject-disjoint splits gains \textbf{4.5 points of
balanced accuracy} (95\% CI 2.3--7.0), and the gain is concentrated almost entirely in the
two rare, clinically important grades ($+9.9$ and $+9.0$ points) while being absent from
the two majority grades. We release the scripts that reproduce every number from public
data.
\end{abstract}
