...
Theirs is a lesion-compositing GAN trained on ACNE04 --- a closed-set generator, which by
construction cannot exceed the severity range of its training data --- where ours is a
prompted open-set foundation model. And their evaluation is a user study and downstream
segmentation IoU, with no independent severity grader and no control separating the
generated content from the rebalancing it provides. The measurements this paper reports are
therefore complementary to theirs rather than competing: they demonstrate the intervention,
we measure whether it delivers what it requested.
