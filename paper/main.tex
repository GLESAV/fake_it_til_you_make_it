...
\paragraph{The leak perceptual hashing cannot see.} Neither perceptual hashing nor CLIP
detects \emph{the same subject photographed twice}, which is the failure mode ACNE04's
capture protocol makes most likely. Embedding faces with ArcFace settles it, and the
answer is the largest single finding of the audit: the 1{,}457 images are 604 people, 326
of whom appear more than once and one of whom appears 22 times. A split drawn at the image
level after perceptual and CLIP deduplication --- which is what every published ACNE04
result does, and what our own first attempt did --- leaves 74.5\% of test images sharing an
identity with training; the published folds leave 77.5\%. Grouping by identity first leaves
none.

Priced by holding everything else fixed and varying only how the split was drawn, that is
worth $+4.51$ points of balanced accuracy (95\% CI $[+2.33, +7.04]$, five paired seeds),
which is larger than the entire spread between the four published methods competing on this
benchmark. It is also not spread evenly: essentially zero on the two majority grades and
about ten points on the two rare ones, which hold 57 and 47 distinct people --- the grades
where recognising an individual is worth most, and the grades this paper's tail-augmentation
arm is about. Every result below is therefore reported on subject-disjoint splits. The
identity analysis is set out in full in the companion audit.

\section{Method}
\label{sec:method}

\subsection{Task and data}
ACNE04 \citep{wu2019}: 1{,}457 images, Hayashi severity in four ordinal grades.
Deduplicated by perceptual hash (Hamming $\leq 2$) and embedding cosine ($\geq 0.98$)
before splitting, with whole duplicate clusters assigned to a single split: 56 clusters
covering 115 images, largest cluster three, leaving 1{,}398 groups. Those groups are then
grouped again by ArcFace identity into 604 subjects, and it is the subjects that are split,
for the reason given in \S\ref{sec:audit}. Splits: 948 train / 218 val / 291 test.
...
convention, and every unsealing is appended to an audit log released with the paper. At the
time of writing that log is empty: \textbf{zero unsealings}. Every number in this paper,
including every control and every negative result, is measured on the validation split.
...
The third check is not available to us: none of our candidate external sets --- AcneSCU,
the deduplicated acne subset of Fitzpatrick17k, the acne subset of SCIN --- has been
obtained with severity labels on this scale, so transfer is untested and the criterion is
not discharged. We therefore do not report the inversion as
...
\begin{itemize}
  \item Test set of 291 images, 108/128/30/25 per grade. The worst-case standard
        error of balanced accuracy on a fresh cohort of that size is 3.8 points, and
        4.3 points on the 218-image validation split every number here is measured on.
        The study is powered for roughly 5-point differences and not for 1--2 point
        ones, and effects below that floor are reported as underpowered rather than
        as absent.
